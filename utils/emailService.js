...
// Send a appointment rejection email to customer
const sendAppointmentRejectionEmail = async (customerEmail, customerName, appointment) => {
    try {
        // Create transporter using Gmail or SMTP settings from environment
        const transporter = nodemailer.createTransport({
            service: process.env.EMAIL_SERVICE || 'gmail',
        ...
        const htmlContent = `
            ...
            <head>
                ...
                <style>
                    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; background-color: #f5f5f5; }
                    .email-container { max-width: 650px; margin: 0 auto; background-color: #ffffff; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
                    .header { background: linear-gradient(135deg, #ca3329 0%, #982a2a 100%); color: white; padding: 30px 20px; text-align: center; }
                    .header h1 { margin: 0; font-size: 28px; font-weight: 600; }
                    .header p { margin: 5px 0 0 0; font-size: 14px; opacity: 0.9; }
                    .content { padding: 30px 20px; }
                    .greeting { font-size: 16px; color: #333; margin-bottom: 20px; line-height: 1.6; }
                ...
                <div class="email-container">
                    ...
                    <div class="content">                        
                        ...
                        <div class="confirmation-box">
                            ...
                            <p>We apologize for the inconvenience caused. If you have any further questions or concerns, please don't hesitate to contact us.</p>
                        </div>
                        ...
                        <div class="appointment-details">
                            ...
                            <div class="detail-section">
                                ...
                                <div class="detail-row">
                                    <span class="detail-label">Status:</span>
                                    <span class="detail-value"><strong class="important">Rejected</strong></span>
                                </div>
                                ${appointment?.reasonForRejection ? `<div class="detail-row">
                                    <span class="detail-label">Reason for Rejection:</span>
                                    <span class="detail-value"><strong class="important">${appointment.reasonForRejection}</strong></span>
                                </div>` : ''}
                                ${appointment?.note ? `<div class="detail-row">
                                    <span class="detail-label">Additional Notes:</span>
                                    <span class="detail-value">${appointment.note}</span>
                        ...
                        <p style="margin-top: 25px; color: #333;">
                            Best regards,<br/>
                            <strong>CH Automobile Service Team</strong><br/>
                            <span style="font-size: 12px; color: #666;">Professional Automotive Service Since 2002</span>
                        </p>
                    </div>

                    <!-- Footer -->
                    <div class="footer">
                        <p><strong>CH Automobile</strong> | Drive Without Worries</p>
        ...
        const mailOptions = {
            from: process.env.EMAIL_USER,
            to: customerEmail,
            subject: `Appointment Rejected - ${formattedDate} at ${appointment?.appointmentTime} | CH Automobile`,
            html: htmlContent,
        };

        await transporter.sendMail(mailOptions);
        console.log(`✅ Professional appointment rejection email sent to ${customerEmail}`);
        return true;
    } catch (error) {
